...
bracketlist* sese(int u, int parent){
    ...
    for(pii child : g[u]){
        ...
        if(mark[v]){// back-edge -> will come first in the dfs traversal for the node at greater depth
            // consider = false; // shouldn't consider this bracket list
            ...
            bl1 = new bracketlist(1, depth[v], ed, ed);
            h0 = min(h0, depth[v]);
        ...
        if((u ^ v) == 1){
            ...
            int U = u >> 1;
            if(U == 3727 || U == 62){
                printArgs(u, v, " : ", bl1->sz, bl1->end->id, key);
            }
            ...
            if(st.find(key) != st.end()){
                ...
                if(find_unique_excluding_selfloop(v, w) == 1 && find_unique_excluding_selfloop(w, v) == 1){
                    ...
                    if(U == 3727 || U == 62)printArgs("in", w, v);
        ...
        if(bl1->start){
            ...
            if(bl1->d < depth[u] && depth[v] > depth[u]){
                if(bl1->d < h1){
                    h2 = h1;
                    h1 = bl1->d;
                }else{
    ...
    if(h2 < h0){
        ...
        // if(!exists){
            ...
            bracketlist* bl1 = new bracketlist(1, depth[w], ed, ed);
