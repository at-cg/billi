...
bool mark_nodes(int u, pii& rs, int ty){
    if(DEBUG)printArgs(get_single_label(u, 0), get_label(rs.F, rs.S), ty);

    if(is_bridge[u >> 1] && (!root_leaf_path(rs.F ^ 1, u) || !root_leaf_path(u, rs.S ^ 1)))return false;
    ...
    for(pii child : g_compacted[u ^ 1]){
        int v = child.F;
        if(DEBUG)printArgs(0, get_single_label(v, 0), child.S, tip_start[biedged_connected_comp[rs.S]]);
        ...
        if(DEBUG)printArgs(1, get_single_label(v, 0));
        // if(visit_order[v] == ty || (ty == 1 && visit_order[v] == 0 && to_chk[v >> 1] != 2)){ // second case to take care of the hairpins (not required)
        if(visit_order[v] == ty){
            continue;
        }
            ...
            for(const auto &[key, vec] : canonical_sese){
                ...
                for(int i = 0; i < sz; ){
                    ...
                    for(j = i; j < sz; j++){
                        ...
                        is_valid = mark_nodes(rs.F ^ 1, rs, 0); // do not add the end points, checking only in one direction
                    
                        if(DEBUG) cout << "HI1" << is_valid << endl;
                        if(!is_valid)continue;

                        is_valid &= mark_nodes(rs.S ^ 1, rs, 1);

                        if(DEBUG) cout << "HI2" << is_valid << endl;
                        is_valid &= (counter == 0);

                        if(DEBUG) cout << "HI3" << counter << endl;
                        DEBUG = false;
