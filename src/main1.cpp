...
void mark_bb_nodes(int u, int& stop, pii& rs, int& id){
    // printArgs("u: ", u);
    if((u == stop || u == dual[stop]) && bb_nodes[id].size() != 0)return; // u == dual[stop] -> can go to y complement but then the pair is not cycle equivalent, if size = 0 implies hairpin loop
    if(!end_gene(u, rs)){// only the directed genes \in \tilde{U} set
        upd_node(dual[u], id, true);
    }
    for(pii child : g_processed[u]){
        int v = child.F;
        if(child.S >= tip_start[biedged_connected_comp[rs.F]] || child.S == -1 || v == u)continue; // don't consider the edges added because of tips and the black edges
        if(mark[v]){
        ...
        mark_bb_nodes(dual[v], stop, rs, id); // need to traverse the black edge
    }
}
...
{   
    ...
    {   
        ...
        // ************************************
        // *** Validating the Theorem from both ends ***
        // ************************************
        for(int it = 0; it < 2 && possible_pairs != 0; it++){
            // ************************************
            ...
            // ************************************
            {
                ...
                if(it == 0){
                    ...
                    fill(bb_comp.begin(), bb_comp.end(), -1);
                    for(int i = 0; i < possible_pairs; i++)bb_nodes[i].clear();
                }
                ...
                for(int i = 0; i < possible_pairs; i++){ // space requirement is linear since the inner-most nested bubbles will appear on the top
                    ...
                    mark_bb_nodes(it == 0 ? rs.F : rs.S, it == 0 ? rs.S : rs.F, rs, i); // do not add the end points, checking only in one direction
                }
            ...
            // ************************************
            // *** Marking valid bibubble ends ***
            // ************************************
            {
                for(int i = 0; i < possible_pairs; i++){
                    if(it == 1 && !valid[i])continue;
                    pii rs = canonical_sese[i];
                    for(int u : bb_nodes[i]){
                        ...
                        // if(!end_gene(u, rs)){ // check only for internal nodes
                            ...
                            // if((aux_cc_comp[u] != aux_cc_comp[dual[it == 0 ? rs.F : rs.S]]) || !valid[bb_comp[u]]){
                                valid[i] = false;
                                valid_pairs--;
                                break;
                            }
