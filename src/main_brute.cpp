...
vector<short> to_chk; // which side of gene to check during second pass of brute (0 -> no chk, 1 -> 2 * i, 2 -> 2 * i + 1, 3 -> both)
...
vector<vector<int>> brute_nodes; // the nodes visited during the brute check for valid bibubbles
set<int> gene_to_chk; // U set used for brute
...
void upd_node_brute(int u, int id, int ty){
    brute_visit[u] = ty;
    q.push(u);
    if(ty == 1) brute_nodes[id].pb(u);
}

bool brute(int start, pii& rs, int id){
    while(!q.empty())q.pop();
    q.push(start);
...
{   
    ...
    {   
        ...
        // ************************************
        {
            ...
            brute_nodes.resize(possible_pairs);
            ...
            for(int i = 0; i < possible_pairs; i++){
                ...
                for(int j = 0; j < brute_nodes[i].size(); j++){
                    ...
                    int gid = oid >> 1;
                    if(to_chk[gid] != 0)to_chk[gid] = 3; // both ends
                    else{
                        to_chk[gid] = (oid & 1) + 1;
                    }
                    gene_to_chk.insert(gid);
                }
                
                for(int x : gene_to_chk){
                    bool vd = false;
                    if(to_chk[x] == 1 || to_chk[x] == 3){
                        vd = can_reach(id_in_present_graph[x << 1], rs);
                    }
                    if(to_chk[x] >= 2){
