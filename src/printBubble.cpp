...
vector<vector<string>> to_print = {
    {"QLV01664.1", "-", "QLV01665.1", "-"},
    {"QLV01665.1", "-", "QLV01667.1", "-"},
        ...
        while (getline(f, line)) {
            ...
            if (!tokens.empty()) {
                if (tokens[0] == "S") {
                    ...
                    if(subgraph.find(s1) != subgraph.end() && subgraph.find(s2) != subgraph.end()){
                        cout << line << endl;
                    }
                }
            }
...
map<int, int> id_in_present_graph; // used to find the id in present graph
    ...
    {   
        if(possible_pairs != 0){
            ...
            // ************************************
            {
                ...
                // summarypath = outputdir + "/U_set.txt";
                // freopen(summarypath.c_str(), "w", stdout);

                for(int i = 0; i < possible_pairs; i++){
                    pii rs = canonical_sese[i];
                    for(int j = 0; j < to_print.size(); j++){
                        int n1 = lmap[to_print[j][0]] << 1, n2 = lmap[to_print[j][2]] << 1;
                        string s1 = to_print[j][1], s2 = to_print[j][3];

                        if(s1 == "+")n1++;
                        if(s2 == "-")n2++;

                        int id1 = id_in_present_graph[n1], id2 = id_in_present_graph[n2];
                        
                        if(id1 == rs.F || id1 == rs.S){
                            summarypath = outputdir + "/" + to_string(j) + ".gfa";
                            freopen(summarypath.c_str(), "w", stdout);            
                            subgraph.clear();

                            subgraph.insert(to_print[j][0]); subgraph.insert(to_print[j][2]);
                            for(int uset : bb_nodes[i]){
                                string gene = ilmap[id_in_original_graph[uset] >> 1];
                                subgraph.insert(gene);
                            }

                            for(string x : subgraph){
                                cout << "S\t" << x << "\t*\tLN:i:100" << endl;
                            }
